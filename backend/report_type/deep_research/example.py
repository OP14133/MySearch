...
class DeepResearch:
    ...
    async def generate_serp_queries(self, query: str, num_queries: int = 3) -> List[Dict[str, str]]:
        ...
        messages = [
            {"role": "system", "content": "You are an expert researcher generating mysearch queries."},
            {"role": "user", "content": f"Given the following prompt, generate {num_queries} unique mysearch queries to research the topic thoroughly. For each query, provide a research goal. Format as 'Query: <query>' followed by 'Goal: <goal>' for each pair: {query}"}
        ]
    ...
    async def process_serp_result(self, query: str, context: str, num_learnings: int = 3) -> Dict[str, List[str]]:
        ...
        messages = [
            {"role": "system", "content": "You are an expert researcher analyzing mysearch results."},

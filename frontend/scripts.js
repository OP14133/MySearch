const GPTResearcher = (() => {
  ...
  const listenToSockEvents = () => {
    ...
    // 根据消息的 type 字段执行不同的操作：
    // logs：调用 addAgentResponse 函数，将日志信息添加到输出区域。
    // images：调用 displaySelectedImages 函数，显示接收到的图像。
    // report：调用 writeReport 函数，将报告数据转换为 HTML 并添加到报告容器中。
    // path：调用 updateState 函数更新状态为 finished，并调用 updateDownloadLink 函数更新下载链接。

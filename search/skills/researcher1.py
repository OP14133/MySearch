...
# from MySearch.mysearch.service.DialogueService import DialogueService
# from ...database.schema import WebPageDetailsSchema
...
class ResearchConductor:
    ...
    async def conduct_research(self):
        ...
        # 默认的基于网页的研究
            ...
            research_data = await self.__get_context_by_search(self.researcher.query)
            # print(f"正在向量化存储1——————————")
            # if self.researcher.vector_store:
            #     print(f"正在向量化存储2——————————")
            #     self.researcher.vector_store.load(research_data)
...
#太长了
    ...
    async def __scrape_data_by_query(self, sub_query):
        ...
        #     url = each.get("url")
        #
        #     content = each.get("raw_content")[:16000]
        #     title = each.get("title")
        #     image_urls = [item["url"] for item in each.get("image_urls")]
        #     body = url_body_dict.get(url)
